...
hem='nh'; k=15;
for n=1:8
  myfilenameA = ['../GCM-Files/Alb-Runs-Output/' char(var(n)),'_',hem,'_',partA,sprintf('%03d.txt', k)];
  myfilenameB = ['../GCM-Files/Alb-Runs-Output/' char(var(n)),'_',hem,'_',partB,sprintf('%03d.txt', k)];
...
nhpidata = zeros( 5, 20400);
  ...
  for k = 1:Nens
   myfilenameA = ['../GCM-Files/LE-from-CC/',char(var(n)),'_',hem,'_',partA,sprintf('%03d.txt', k)];
   myfilenameB = ['../GCM-Files/LE-from-CC/',char(var(n)),'_',hem,'_',partB,sprintf('%03d.txt', k)];
   tmpA  = importdata(myfilenameA);
   tmpB  = importdata(myfilenameB);
  ...
  myfilenameC = ['../GCM-Files/LE-from-CC/',char(var(n)),'_',hem,'_',partC];
...
for n=1:5
  eval([char(var(n)),'_ens=squeeze(mean(',char(var(n)),',1));']);
end

yrs_exp=1997+(0:612)/12;
